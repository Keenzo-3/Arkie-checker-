...
def load_config() -> dict:
    ...
    # Default config — secrets read from env vars
    _admin_ids_raw = os.environ.get("ADMIN_IDS", "")
    _admin_ids = [int(x.strip()) for x in _admin_ids_raw.split(",") if x.strip().isdigit()]
    return {
        "bot_token": os.environ.get("BOT_TOKEN", ""),
        "admin_ids": _admin_ids,
        "channel_username": os.environ.get("CHANNEL_USERNAME", "leakbotsrc"),
        ...
        "gcash_number": "09933119204",
        "gcash_name": "J.O",
        "notify_admin_on_hit": True,
        "welcome_message": (
            "Welcome to MITZ CODM Checker! 🎮\n"
            "Buy a key via GCash — fast and cheap.\n"
            "Questions? Just message the admin. Enjoy!"
        ),
    }
